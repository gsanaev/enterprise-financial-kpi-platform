...
def get_paths() -> Paths:
    project_root = Path(__file__).resolve().parents[2]  # go up from src/package/file.py
...
def generate_dim_customer(
    ...
    # Random acquisition dates spread across first 3 years
    acq_start = start
    ...
    # Random churn times (or no churn)
    churn_flags = rng.random(n_customers) < annual_churn_rate
    churn_offsets = rng.exponential(scale=365 / annual_churn_rate, size=n_customers)
    churn_dates = acq_dates + pd.to_timedelta(churn_offsets, unit="D")
    churn_dates = np.where(churn_flags, churn_dates, np.datetime64('NaT'))
    churn_dates = pd.to_datetime(churn_dates)

    # Ensure churn dates are not after end date (NaT stays NaT)
    churn_dates = pd.to_datetime(churn_dates)
    churn_dates = churn_dates.where(churn_dates <= end, end)

    segments = ["Retail", "SME", "Corporate"]
    regions = ["North", "South", "West", "East", "Central", "International"]

    segment_probs = [0.6, 0.3, 0.1]  # more Retail
